...
class TrendTracker:
    ...
    def get_alert_message(self):
        """Trả về thông báo báo động dựa trên total_trend"""
        if self.total_trend >= 90:
            return f"💀💀💀 Báo động cấp độ 3, Giá đi {self.total_trend}USD rồi"
        elif self.total_trend >= 70:
            return f"🚨🚨 Báo động cấp độ 2, Giá đi {self.total_trend}USD rồi"
        elif self.total_trend >= 40:
            return f"⚠️ Báo động cấp độ 1, Giá đi {self.total_trend}USD rồi"
        else:
            return None
    ...
    def update(self, price: float):
        ...
        if abs(delta) < 1:
            if self.trend == "up":
                messages.append(f"🥱 Giá ít biến động: {price} ({delta:+.2f}USD), tổng tăng: {self.total_trend:.2f} USD")
            elif self.trend == "down":
                messages.append(f"🥱 Giá ít biến động: {price} ({delta:+.2f}USD), tổng giảm: {self.total_trend:.2f} USD")
            elif self.trend == "sideway":
                messages.append(f"🥱 Giá ít biến động: {price} ({delta:+.2f}USD)")
            else:
                messages.append(f"Giá ít biến động: {delta:+.2f} USD")
        elif delta > 0:
            if self.trend == "up":
                messages.append(f"Giá:{price} tăng {delta:.2f} USD, tổng tăng: {self.total_trend:.2f} USD")
            elif self.trend == "sideway":
                messages.append(f"Giá:{price} tăng {delta:.2f} USD")
            else:
                messages.append(f"Giá:{price} tăng {delta:.2f} USD")
        elif delta < 0:
            if self.trend == "down":
                messages.append(f"Giá:{price} giảm {abs(delta):.2f} USD, tổng giảm: {self.total_trend:.2f} USD")
            elif self.trend == "sideway":
                messages.append(f"Giá:{price} giảm {abs(delta):.2f} USD")
            else:
                messages.append(f"Giá:{price} giảm {abs(delta):.2f} USD")
        ...
        if self.trend == "sideway":
            ...
            if self.total_trend >= 10 and updated:
                if price == self.high:
                    ...
                    self.total_trend = self.high - self.low
                    messages.append(f"Giá tăng hơn 10 USD ,tổng tăng: {self.total_trend:.2f} USD")
                elif price == self.low:
                    self.trend = "down"
                    self.start_price = self.high
                    self.extreme_price = self.low
                    self.total_trend = self.start_price - self.extreme_price
                    messages.append(f"Giá giảm hơn 10 USD ,tổng giảm: {self.total_trend:.2f} USD")
            else:
                messages.append(f"Giá sideway, range tổng: {self.total_trend:.2f} USD")
            ...
            if self.pre_reset_trend == "up":
                if price > self.extreme_price:
                    ...
                    self.reset_price = None
                    messages = [f"🔼 Giá {price} phá đỉnh cũ, tổng tăng: {self.total_trend:.2f} USD"]
                    ...
                    self.total_trend = self.start_price - self.extreme_price
                    self.reset_price = None
                    messages = [f"Giá {price} tiếp tục giảm, tổng giảm: {self.total_trend:.2f} USD"]
                else:
                    messages = [f"Giá ít biến động {delta:+.2f} USD, chưa xác định xu hướng mới"]
                ...
                if price < self.extreme_price:
                    ...
                    self.total_trend = self.start_price - self.extreme_price
                    self.reset_price = None
                    messages = [f"🔻 Giá phá đáy, tổng giảm: {self.total_trend:.2f} USD"]
                elif price > self.reset_price:
                    self.trend = "up"
                    self.start_price = self.extreme_price
                    self.extreme_price = price
                    self.total_trend = self.extreme_price - self.start_price
                    self.reset_price = None
                    messages = [f"Giá tiếp tục tăng, tổng tăng: {self.total_trend:.2f} USD"]
                else:
                    messages = [f"Giá ít biến động {delta:+.2f} USD, chưa xác định xu hướng mới"]
            ...
            if price > self.extreme_price:
                self.extreme_price = price
                self.total_trend = self.extreme_price - self.start_price
                messages = [f"📈 Giá tạo đỉnh mới ({price}), tổng tăng: {self.total_trend:.2f} USD"]
            else:
                ...
                if pull_pct >= 100:
                    ...
                    self.extreme_price = price
                    self.total_trend = self.start_price - self.extreme_price
                    messages = [f"🤩 AN TOÀN! Giá quay về đáy cũ, tổng giảm: {self.total_trend:.2f} USD"]
                elif pull_pct >= 40:
                    ...
                    self.total_trend = 0
                    messages = [
                        f"🤩 AN TOÀN! Giá giảm {pull_amt:.2f} USD, Pullback {pull_pct:.2f}% sau chuỗi tăng: {old_total:.2f} USD"]
                else:
                    messages = [
                        f"🥱 Giá giảm {pull_amt:.2f} USD, Pullback {pull_pct:.2f}% sau chuỗi tăng: {self.total_trend:.2f} USD"]

        elif self.trend == "down":
            if price < self.extreme_price:
                self.extreme_price = price
                self.total_trend = self.start_price - self.extreme_price
                messages = [f"Giá tạo đáy mới ({price}), tổng giảm: {self.total_trend:.2f} USD"]
            else:
                ...
                if pull_pct >= 100:
                    ...
                    self.extreme_price = price
                    self.total_trend = self.extreme_price - self.start_price
                    messages = [f"🤩 AN TOÀN! Giá quay về đỉnh cũ, tổng tăng: {self.total_trend:.2f} USD"]
                elif pull_pct >= 40:
                    ...
                    self.total_trend = 0
                    messages = [
                        f" 🤩 AN TOÀN! Giá tăng {pull_amt:.2f} USD, Pullback {pull_pct:.2f}% sau chuỗi giảm: {old_total:.2f} USD"]
                else:
                    messages = [
                        f"🥱 Giá tăng {pull_amt:.2f} USD, Pullback {pull_pct:.2f}% sau chuỗi giảm: {self.total_trend:.2f} USD"]
